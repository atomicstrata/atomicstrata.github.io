...
Append-only memory rots. Every time a user restates something it becomes another row, contradictions pile up, and retrieval hands back three versions of the same fact. AtomicMemory reconciles each write against what it already knows, so memory stays consistent as facts change.
