...
Filecoin data should be treated as publicly retrievable by CID unless it is encrypted before upload. AtomicMemory supports an optional raw-content codec for managed document uploads:
...
With `RAW_CONTENT_CODEC=aes_gcm`, core encrypts raw document bytes before the storage provider receives them. With the default `RAW_CONTENT_CODEC=none`, the provider receives plaintext bytes. Do not use plaintext Filecoin storage for sensitive data.
...
-   Filecoin/IPFS storage is content-addressed and publicly retrievable by CID; enable `RAW_CONTENT_CODEC=aes_gcm` before using Filecoin for sensitive raw bytes
