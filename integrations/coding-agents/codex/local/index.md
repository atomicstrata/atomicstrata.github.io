...
In the source-backed flow, `ATOMICMEMORY_MCP_SERVER_BIN` points Codex at the built local MCP server. Provider, URL, credentials, and scope still come from the shell or Codex environment.
...
-   **Connection failed** - verify `ATOMICMEMORY_MCP_SERVER_BIN`, `ATOMICMEMORY_PROVIDER`, `ATOMICMEMORY_API_URL`, `ATOMICMEMORY_API_KEY` when your provider requires auth, and at least one `ATOMICMEMORY_SCOPE_*` value are visible to Codex.
