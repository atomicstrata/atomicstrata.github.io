# Codex

> Agent index: [llms.txt](/llms.txt)

AtomicMemory integrates with Codex through the shared MCP server and a memory protocol skill that handles retrieval, durable writes, and handoff snapshots.

## Deployment targets

### Local

Use this when you want the self-managed Codex path and need the current clone-from-source workflow.

See [Codex Local](/integrations/coding-agents/codex/local).

### Cloud

Coming Soon

Cloud deployment details for Codex are not documented yet. This page will describe the hosted configuration, authentication, and rollout model once those details are finalized.

See [Codex Cloud](/integrations/coding-agents/codex/cloud).
