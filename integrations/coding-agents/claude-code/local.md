...
Give Claude Code persistent, cross-session memory backed by Open Source. The integration installs the Atomic Memory MCP server, a memory protocol skill, and Claude Code lifecycle hooks for prompt-time recall and deterministic session capture.
...
Log in to Claude Code, then start local core with Claude Code account auth and local embeddings:
...
The plugin defaults to the local core URL and local quickstart key.
...
The Claude Code plugin is discovered from the public Atomic Memory monorepo. Source and development live in the [`plugins/claude-code`](https://github.com/atomicstrata/atomicmemory/tree/main/plugins/claude-code) package.
...
The plugin connects to the configured Open Source service and exposes memory tools.
...
Use local core mode for personal Claude Code memory. Start Core with Claude Code account auth, then install the plugin so Claude Code can call the MCP server and lifecycle hooks. The core process uses `LLM_PROVIDER=claude-code` for extraction without a separate Anthropic API key.
...
### External Open Source mode

Hosted Atomic Memory coming soon

Hosted Atomic Memory for Claude Code is planned but not yet available. For now, use this mode with your own self-hosted Open Source deployment.

Use external mode when a team or production deployment runs Open Source for you. Configure the service URL and token, and let the hosted service own LLM credentials, storage, backups, and policy.
...
After creating the project `.mcp.json`, [approve/trust the project MCP server and verify it with `claude mcp list`](#claude-code-does-not-show-atomic-memory-mcp-tools) before starting a Claude Code session that needs Atomic Memory tools.
...
The plugin ships hooks by default. If you maintain your own Claude Code config, the Atomic Memory CLI can generate equivalent hook snippets:
...
| Setting | Default |
| --- | --- |
| Runtime | User-managed Core |
| Service URL | `http://127.0.0.1:17350` unless `ATOMICMEMORY_API_URL` is set |
| User scope | Claude Code user/session context, then local fallback |
| Agent scope | `claude-code` |
| Capture level | `balanced` |
| Memory extraction provider | Configured on the Core process |
...
Claude Code hooks and the MCP server use `ATOMICMEMORY_*` values for service URL, scope, and capture policy. When omitted, the local defaults are `ATOMICMEMORY_API_URL=http://127.0.0.1:17350` and `ATOMICMEMORY_API_KEY=local-dev-key`.

The quickstart starts local core with Claude Code account auth:
...
The `claude-code` provider routes through Anthropic's Agent SDK, consumes the user's Claude Code / Claude account limits according to Anthropic policy, and is for personal local use.

When you are running your own shared Open Source runtime instead of using Claude Code's local authenticated session, configure an API-backed extraction provider:
...
| `ATOMICMEMORY_API_URL` | MCP + hooks | Open Source service URL. Defaults to local core at `http://127.0.0.1:17350` when omitted. |
| `ATOMICMEMORY_API_KEY` | MCP + hooks | API key for an external Open Source service. |
...
### Claude Code does not show Atomic Memory MCP tools

If you configured Open Source through a project `.mcp.json`, Claude Code may require the project MCP server to be approved/trusted before the tools are available.
...
| Local runtime does not start | Confirm Claude Code can run local plugin commands and check Open Source plugin logs. |
| `claude-code` provider fails | Run `claude auth login`, then verify with `claude auth status --json`, `claude doctor`, or `claude --version`. Use `LLM_PROVIDER=anthropic` and `ANTHROPIC_API_KEY` for hosted or team deployments. |
